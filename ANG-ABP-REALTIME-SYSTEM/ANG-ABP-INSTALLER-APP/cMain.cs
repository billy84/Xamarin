...
namespace ANG_ABP_INSTALLER_APP
{
    ...
    class cMain
    {
        ...
        /// </summary>
        public static cProjectTable p_cInstallInputScreenData = null;


        /// <summary>
        /// For recording notes input in the survey input screen, is used to then redisplay when the user navigates back to the page.
        /// </summary>
        public static List<cProjectNotesTable> p_cInstallInputProjectNotes = null;
        ...
        /// <summary>
        /// Flag to indicate that installers are being checked for
        /// </summary>
        private static bool m_bCheckingInstallers = false;
        ...
        //        cMain.p_cSettings.DisplayMessage += cSync_DisplayMessage;
                ...
                if (pgPage is MainPage)
                {
                    ...
                    if (tbOnline != null)
                    {
                        ...
                        if (bOnline == true)
                        {
                            ...
                            //Check for installers update
                            await cMain.CheckForInstallers();
        ...
        public static string ReturnLastSyncString(DateTime? v_dSyncDate)
        {
            ...
            try
            {
                ...
                int iUnits = cMain.p_cDataAccess.GetNumberOfUnitUpdatesPending();

                sDisplayText.Append(Environment.NewLine);
                sDisplayText.Append("(" + (iUploads + iNotes + iFiles + iUnits).ToString() + ") changes waiting for upload.");
        ...
        /// <summary>
        /// v1.0.11 - Return delivery display date.
        /// </summary>
        /// <param name="v_dDelDate"></param>
        /// <returns></returns>
        public static string ReturnDeliveryDisplayDate(DateTime? v_dDelDate)
        {

            try
            {

                if (v_dDelDate.HasValue == false || v_dDelDate.Value.Year == 1900)
                {
                ...
                {
                    return cMain.ReturnDisplayDate(v_dDelDate.Value);
                }
        ...
        /// <summary>
        /// Should I check for installers
        /// </summary>
        /// <returns></returns>
        private static bool ShouldICheckForInstallers()
        {
            ...
            try
            {
                ...
                int iDaysBetweenChecks = Convert.ToInt32(cMain.GetAppResourceValue("CheckInstallersDaysBetweenChecks").ToString());

                cAppSettingsTable cSettings = cMain.p_cDataAccess.ReturnSettings();
                if (cSettings != null)
                {

                    if (cSettings.LastInstallersCheckDateTime.HasValue == true)
                    {
                        TimeSpan tsDiff = DateTime.Now.Subtract(cSettings.LastInstallersCheckDateTime.Value);
                        if (tsDiff.TotalDays >= iDaysBetweenChecks)
                        {
                            bShouldICheck = true;
        ...
        /// </summary>
        /// <returns></returns>
        public async static Task CheckForInstallers()
        {
            ...
            try
            {
                ...
                if (cMain.m_bCheckingInstallers == true) { return; }
                cMain.m_bCheckingInstallers = true;

                 bool bCheck = cMain.ShouldICheckForInstallers();
                 if (bCheck == true)
                 {

                     cAppSettingsTable cSettings = cMain.p_cDataAccess.ReturnSettings();

                     DateTime dLastUpdate = DateTime.Now;

                     if (cSettings != null)
                     {
                         if (cSettings.LastInstallersUpdateDateTime.HasValue == true)
                         {
                             dLastUpdate = cSettings.LastInstallersUpdateDateTime.Value;
                         }
                     }

                     cAX = new cAXCalls();

                     ANG_ABP_SURVEYOR_APP_CLASS.wcfAX.FetchInstallersResult fiResult = await cAX.FetchInstallersList(dLastUpdate);

                     if (fiResult != null)
                     {

                         if (fiResult.bSuccessfull == true)
                         {

                             if (fiResult.Installers != null)
                             {

                                 if (fiResult.Installers.Count > 0)
                                 {

                                    bool bClearedOK = cMain.p_cDataAccess.ClearInstallersTable();
                                    if (bClearedOK == true)
                                    {

                                        bool bSavedOK = cMain.p_cDataAccess.UpdateInstallersTable(fiResult.Installers);

                                        if (bSavedOK == true)
                                        {

                                            cSettings.LastInstallersUpdateDateTime = fiResult.bLastUpdateDate;
                                            cSettings.LastInstallersCheckDateTime = DateTime.Now;
                 ...
                 cMain.m_bCheckingInstallers = false;
        ...
        static void cSync_SubProjectStatusUpdate(object sender, cSyncEventParamProjectStatus e)
        {
            ...
            try
            {
                ...
                cMain.UpdateSyncPageProjectStatus(e.PrevProjectNo, e.NextProjectNo, e.PrevProjectSubProjectsAdded,e.PrevProjectSubProjectsDeleted);
        ...
        public static void UpdateSyncPageProjectStatus(string v_sPrevProjNo, string v_sNextProjNo, int v_iPrevProjSubProjectsAdded, int v_iPrevProjSubProjectsDeleted)
        {
        ...
        public static string ReturnSubProjectWillNotDisplayOnFrontScreenMessage(List<string> v_lstJobs)
        {
            ...
            try
            {
                ...
                string sInstallStatusName = cMain.p_cDataAccess.GetEnumValueName("ProjTable", "Mxm1002InstallStatus", cSettings.p_iInstallStatus_InstalledFully);                

                sbMsg.Append(Environment.NewLine);
                sbMsg.Append("The status needed to display on the front screen are:");
                sbMsg.Append(Environment.NewLine);
                sbMsg.Append("Install status must not be: " + sInstallStatusName);
                sbMsg.Append(Environment.NewLine);
                sbMsg.Append("Appointment must be confirmed.");
                sbMsg.Append(Environment.NewLine);
        ...
        /// <summary>
        /// Checks too see if sub project status is such that it will not show on front screen.
        /// </summary>
        /// <param name="v_iSubProjectProgressStatus"></param>
        /// <param name="v_iSubProjectInstallStatus"></param>
        /// <returns></returns>
        public static bool WillSubProjectDisplayOnFrontScreen(int? v_iConfirmedAppointmentIndicator, int v_iSubProjectInstallStatus)
        {
            ...
            try                                  
            {
                ...
                if (v_iConfirmedAppointmentIndicator.HasValue == true)
                {
                    if (v_iConfirmedAppointmentIndicator.Value == 1)
                    {                    
                        if (v_iSubProjectInstallStatus != cSettings.p_iInstallStatus_InstalledFully)
                        {
                            return true;
                        }   
                    }
        ...
        /// <summary>
        /// Populate installers drop down.
        /// </summary>
        /// <param name="r_cmbCombo"></param>
        public static void PopulateInstallersDropDown(ref ComboBox r_cmbCombo)
        {
            ...
            try
            {
                ...
                List<cInstallersTable> lInstallers = cMain.p_cDataAccess.FetchAllInstallers();

                ComboBoxItem cmbItem = new ComboBoxItem();
                cmbItem.Content = cSettings.p_sPleaseChoose;
                cmbItem.Tag = -1;

                r_cmbCombo.Items.Add(cmbItem);

                foreach (cInstallersTable cInstall in lInstallers)
                {

                    cmbItem = new ComboBoxItem();
                    cmbItem.Content = cInstall.Name;
                    cmbItem.Tag = cInstall.AccountNum;

                    r_cmbCombo.Items.Add(cmbItem);

                }

                r_cmbCombo.SelectedIndex = 0;
            ...
            catch (Exception ex)
            {
                ...
                /// <summary>
        /// v1.0.19 - Routine to check if sub project has incomplete remakes, also displays message to user.
        /// </summary>
        /// <param name="v_sSubProjectNo"></param>
        /// <returns></returns>
        public async static Task<bool> DoesSubProjectHaveInCompleteRemakes(string v_sSubProjectNo)
        {

            try
            {

                return await cMain.DoesSubProjectHaveInCompleteRemakes(v_sSubProjectNo, string.Empty);
        ...
        /// <param name="v_sSubProjectNo"></param>
        /// <returns></returns>
        public async static Task<bool> DoesSubProjectHaveInCompleteRemakes(string v_sSubProjectNo,string v_sExtraMessageText)
        {
            bool bAnyRemakes = false;
            try
            {

                List<cSubProjectResult> lSubProjects = cMain.p_cDataAccess.FetchInCompleteRemakesForSubProject(v_sSubProjectNo);
                if (lSubProjects != null)
                {

                    StringBuilder sbSubProjects = new StringBuilder();

                    foreach (cSubProjectResult srSubProject in lSubProjects)
                    {
                        bAnyRemakes = true;

                        sbSubProjects.Append(srSubProject.SubProjectNo + " - " + cMain.RemoveNewLinesFromString(srSubProject.DeliveryStreet) +
                                             Environment.NewLine);
                    ...
                    if (bAnyRemakes == true)
                    {
                        ...
                        string sMessage = "Cannot update sub project (" + v_sSubProjectNo + ") as incomplete remakes exist:";

                        sMessage += Environment.NewLine + Environment.NewLine;

                        sMessage += sbSubProjects.ToString();

                        if (v_sExtraMessageText.Length > 0)
                        {

                            sMessage += Environment.NewLine + v_sExtraMessageText;

                        }

                        await cSettings.DisplayMessage(sMessage, "Incomplete Remakes");
