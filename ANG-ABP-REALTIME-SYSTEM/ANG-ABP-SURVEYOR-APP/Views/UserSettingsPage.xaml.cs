    ...
    public sealed partial class UserSettingsPage : Page
    {
        ...
        async void MainPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                bool bAllowed = Windows.System.UserProfile.UserInformation.NameAccessAllowed;
                if (bAllowed == false)
                {
                    this.txtUserName.Text = "Display Name";
                }
                else
                {
        ...
        private async void btnBrowsePicture_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                List<StorageFile> sfFiles = new List<StorageFile>();
                sfFiles.Add(sfFile);

                // 1.0.18
                sfFiles = await cSettings.CopyFilesToTemp(sfFiles);
                foreach (StorageFile sfFileCopy in sfFiles)
                {

                    this.m_cSettings.ProfilePicPath = sfFileCopy.Path;
                    this.UpdateImage();
