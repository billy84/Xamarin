...
''' </summary>
Public NotInheritable Class SelectPhotos
    Inherits Page
...
#End Region
    ...
    Private Async Sub btnBrowse_Click(sender As Object, e As RoutedEventArgs) Handles btnBrowse.Click
        ...
        Dim lsPhotos As New List(Of cPhoto)
        Try


            Dim tn As New BitmapImage()
            Dim sFile As StorageFile = Await StorageFile.GetFileFromApplicationUriAsync(New System.Uri("ms-appx:///assets/house.jpg"))

            Dim sImage As IRandomAccessStream = Await sFile.OpenAsync(FileAccessMode.Read)

            Dim cPic As New cPhoto
            cPic.MyImage = New BitmapImage
            Await cPic.MyImage.SetSourceAsync(sImage)

            cPic.Comment = String.Empty

            lsPhotos.Add(cPic)

            sFile = Await StorageFile.GetFileFromApplicationUriAsync(New System.Uri("ms-appx:///assets/house2.jpg"))
            sImage = Await sFile.OpenAsync(FileAccessMode.Read)
            ...
            Me.lvPhotos.ItemsSource = lsPhotos
